...
# The Vertex SDK conflicts with the GPU stack's grpcio/protobuf, so it lives in its own venv.
# Create it once and install the (lightweight, cloud-only) Vertex deps there.
if [ ! -d .venv-vertex ]; then
  echo "creating isolated Vertex venv (.venv-vertex)..."
  uv venv --python 3.11 .venv-vertex
  uv pip install --python .venv-vertex/bin/python -r setup/requirements-vertex.txt
fi

.venv-vertex/bin/python rung1-vendor-ft/finetune_vertex.py --n 2000 --epochs 3
