...
`ANTHROPIC_API_KEY` is set, also records a frontier reference line (Opus 4.8, few-shot+RAG).
...
- **Frontier reference (Opus 4.8, few-shot+RAG)** is "rent the best model and prompt it well" -- the
  number a real customer starts from and the ceiling the small owned model chases. It is a
  reference, not the fine-tuning baseline. The compelling story is not "our 3B beats Opus" (it
  likely won't on raw EX); it is how close a cheap model you own can get on *this* task, and what
  owning it then lets you do (serve it cheaply, add constrained decoding, add a verifier).

Because Anthropic exposes an OpenAI-compatible endpoint, the frozen `eval.py` hits Opus unchanged;
the frontier line is just a labeled invocation, no new code.
