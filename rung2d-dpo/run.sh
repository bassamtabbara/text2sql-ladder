...
vllm serve checkpoints/full-ft --port 8000 --max-model-len 8192 --served-model-name full-ft &
VLLM_PID=$!
...
vllm serve checkpoints/dpo --port 8000 --max-model-len 8192 --served-model-name dpo &
VLLM_PID=$!
until curl -sf http://localhost:8000/v1/models >/dev/null 2>&1; do sleep 3; done
